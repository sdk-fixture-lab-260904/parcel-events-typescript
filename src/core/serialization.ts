import { serializeParameter, type ParameterSerialization } from './parameter-serialization.js';
/**
...
export function encodeQuery(
  ...
  styles?: Readonly<Record<string, QueryStyle | ParameterSerialization>>,
): string {
  const pairs: string[] = [];
  for (const [key, value] of Object.entries(params)) {
    if (value === undefined) continue;
    const style = styles?.[key] ?? 'form';
    if (typeof style === 'object') pairs.push(serializeParameter(key, value, 'query', style));
    else if (value !== null) encodeParam(pairs, key, value, style);
  }
  return pairs.join('&');
}
...
export type MultipartFieldValue = Blob | Uint8Array | UploadPart | QueryPrimitive;
...
export function lowercaseKeys(record: Readonly<Record<string, string>> | undefined): Record<string, string> {
