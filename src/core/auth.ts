...
/** Evaluated per attempt, allowing short-lived credentials without rebuilding the client. */
export type Credential = string | (() => string | Promise<string>);
async function credentialValue(value: Credential): Promise<string> {
  const resolved = typeof value === 'function' ? await value() : value;
  if (typeof resolved !== 'string' || resolved.length === 0) throw new Error('Credential provider must return a non-empty string');
  return resolved;
}
export function apiKeyAuth(options: ApiKeyAuthOptions): AuthProvider {
  return { async authorize(): Promise<AuthMaterial> {
    const key = await credentialValue(options.key);
    return options.placement === 'header'
      ? { headers: { [options.name.toLowerCase()]: key } }
      : { query: { [options.name]: key } };
  } };
}
export function bearerAuth(token: Credential): AuthProvider {
  return { async authorize(): Promise<AuthMaterial> {
    return { headers: { authorization: `Bearer ${await credentialValue(token)}` } };
  } };
}
