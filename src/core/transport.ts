...
import { protectBody } from './response-body.js';
...
export async function performFetch(fetchFn: KernelFetch, request: TransportRequest): Promise<Response> {
  ...
  const failure = (): Error => isAborted(request.signal) ? new APIUserAbortError() : new APIConnectionTimeoutError(request.timeoutMs);
  try {
    ...
    return protectBody(response, controller.signal, failure, cleanup);
  } catch (error) {
    cleanup();
    if (isAborted(request.signal)) throw new APIUserAbortError();
